...
public class PaletteColorModel extends ColorModel {
    ...
    public PaletteColorModel(int bits, int dataType, ColorSpace cs, DataSet ds) {
        super(bits, opaqueBits, cs, false, false, OPAQUE, dataType);
        int[] rDesc = lutDescriptor(ds, AttributeTag.RedPaletteColorLookupTableDescriptor);
        int[] gDesc = lutDescriptor(ds, AttributeTag.GreenPaletteColorLookupTableDescriptor);
        int[] bDesc = lutDescriptor(ds, AttributeTag.BluePaletteColorLookupTableDescriptor);
        byte[] r = lutData(ds, rDesc, AttributeTag.RedPaletteColorLookupTableData, AttributeTag.SegmentedRedPaletteColorLookupTableData);
        byte[] g = lutData(ds, gDesc, AttributeTag.GreenPaletteColorLookupTableData, AttributeTag.SegmentedGreenPaletteColorLookupTableData);
        byte[] b = lutData(ds, bDesc, AttributeTag.BluePaletteColorLookupTableData, AttributeTag.SegmentedBluePaletteColorLookupTableData);
        lut = LUT.create(bits, r, g, b, rDesc[1], gDesc[1], bDesc[1]);
    }

    private int[] lutDescriptor(DataSet ds, AttributeTag descTag) {
        int[] desc = ds.intValuesOf(descTag);
        if (desc == null) {
    ...
    private byte[] lutData(DataSet ds, int[] desc, int dataTag, int segmTag) {
    ...
    public WritableRaster createCompatibleWritableRaster(int w, int h) {
        return Raster.createInterleavedRaster(pixel_bits <= 8 ? DataBuffer.TYPE_BYTE : DataBuffer.TYPE_USHORT, w, h, 1, null);
    }
    ...
    public BufferedImage convertToIntDiscrete(Raster raster) {
        ...
        ColorModel cm = new DirectColorModel(getColorSpace(), 24, 0xff0000, 0x00ff00, 0x0000ff, 0, false, DataBuffer.TYPE_INT);
    ...
    private static abstract class LUT {
        ...
        public static LUT create(int bits, byte[] r, byte[] g, byte[] b, int rOffset, int gOffset, int bOffset) {
            ...
            return r.length == g.length && g.length == b.length && rOffset == gOffset && gOffset == bOffset ? new Packed(bits, r, g, b, rOffset) : new PerColor(bits, r, g, b, rOffset, gOffset,
                    bOffset);
        }
        ...
        static class PerColor extends LUT {
            ...
            @Override
            public int getRGB(int pixel) {
                return 0xff000000 | (value(pixel, rOffset, r) << 16) | (value(pixel, gOffset, g) << 8) | (value(pixel, bOffset, b));
            }
